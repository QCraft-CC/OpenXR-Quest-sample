...
public:
    ...
    bool
    BInit(const vr::RenderModel_t &vrModel, const vr::RenderModel_TextureMap_t &vrDiffuseTexture);
...
//---------------------------------------------------------------------------------------------------------------------
bool GetDigitalActionRisingEdge(vr::VRActionHandle_t action,
        ...
        *pDevicePath = vr::k_ulInvalidInputValueHandle;
        if (actionData.bActive) {
            ...
            if (vr::VRInputError_None ==
                vr::VRInput()->GetOriginTrackedDeviceInfo(actionData.activeOrigin, &originInfo,
                                                          sizeof(originInfo))) {
...
//---------------------------------------------------------------------------------------------------------------------
bool GetDigitalActionFallingEdge(vr::VRActionHandle_t action,
                                 vr::VRInputValueHandle_t *pDevicePath = nullptr) {
    vr::InputDigitalActionData_t actionData;
    vr::VRInput()->GetDigitalActionData(action, &actionData, sizeof(actionData),
                                        vr::k_ulInvalidInputValueHandle);
...
CMainApplication::CMainApplication(int argc, char *argv[])
        : // m_pCompanionWindow(NULL), m_pContext(NULL), m_nCompanionWindowWidth(640), m_nCompanionWindowHeight(320),
        m_pContext(NULL),
        m_unSceneProgramID(0), m_unCompanionWindowProgramID(0),
        m_unControllerTransformProgramID(0), m_unRenderModelProgramID(0), m_pHMD(NULL),
        m_bDebugOpenGL(false), m_bVerbose(false), m_bPerf(false), m_bVblank(false),
        m_bGlFinishHack(true), m_glControllerVertBuffer(0), m_unControllerVAO(0), m_unSceneVAO(0),
        m_nSceneMatrixLocation(-1), m_nControllerMatrixLocation(-1),
        m_nRenderModelMatrixLocation(-1), m_iTrackedControllerCount(0),
        m_iTrackedControllerCount_Last(-1), m_iValidPoseCount(0), m_iValidPoseCount_Last(-1),
        m_iSceneVolumeInit(20), m_strPoseClasses(""), m_bShowCubes(true) {
...
//-----------------------------------------------------------------------------
std::string
GetTrackedDeviceString(vr::TrackedDeviceIndex_t unDevice, vr::TrackedDeviceProperty prop,
                       vr::TrackedPropertyError *peError = NULL) {
    uint32_t unRequiredBufferLen = vr::VRSystem()->GetStringTrackedDeviceProperty(unDevice, prop,
                                                                                  NULL, 0, peError);
...
bool CMainApplication::BInit() {
    ...
    // m_pCompanionWindow = SDL_CreateWindow("hellovr", nWindowPosX, nWindowPosY,
    ...
    if ((display = eglGetDisplay(EGL_DEFAULT_DISPLAY)) == EGL_NO_DISPLAY) {
        logf(ANDROID_LOG_ERROR, "eglGetDisplay() returned error %d", eglGetError());
        return false;
    }
    if (!eglInitialize(display, 0, 0)) {
        logf(ANDROID_LOG_ERROR, "eglInitialize() returned error %d", eglGetError());
        return false;
    }
    if (!eglBindAPI(EGL_OPENGL_ES_API)) {
        logf(ANDROID_LOG_ERROR, "Failed to bind GL API: %d", eglGetError());
        return false;
    }
    ...
    if (!eglChooseConfig(display, attribs, configs, 10, &numConfigs)) {
        logf(ANDROID_LOG_ERROR, "eglChooseConfig() returned error %d", eglGetError());
        return false;
    }
    ...
    if (!(m_pContext = eglCreateContext(display, config, 0, attrs))) {
        logf(ANDROID_LOG_ERROR, "eglCreateContext() returned error %d", eglGetError());
        return false;
    }
    if (!eglMakeCurrent(display, surface, surface, m_pContext)) {
        logf(ANDROID_LOG_ERROR, "eglMakeCurrent() returned error %d", eglGetError());
        return false;
    }
    ...
    XrGraphicsBindingOpenGLESAndroidKHR *binding = new XrGraphicsBindingOpenGLESAndroidKHR;
    *binding = {XR_TYPE_GRAPHICS_BINDING_OPENGL_ES_ANDROID_KHR};
    binding->display = display;
    binding->config = config;
    binding->context = m_pContext;
    OpenComposite_Android_GLES_Binding_Info = binding;



    // if (!eglQuerySurface(display, surface, EGL_WIDTH, &width) ||
    //     !eglQuerySurface(display, surface, EGL_HEIGHT, &height)) {
    //     logf(ANDROID_LOG_ERROR, "eglQuerySurface() returned error %d", eglGetError());
    //     return false;
    // }
...
// 		m_SecondsTimer.start(1000, this);
    ...
    //vr::VRInput()->SetActionManifestPath(Path_MakeAbsolute("../hellovr_actions.json",
    //                                                       Path_StripFilename(
    //                                                               Path_GetExecutablePath())).c_str());
    vr::VRInput()->SetActionManifestPath("input/hellovr_actions.json");
...
bool CMainApplication::HandleInput() {
    ...
    if (GetDigitalActionRisingEdge(m_actionTriggerHaptic, &ulHapticDevice)) {
        if (ulHapticDevice == m_rHand[Left].m_source) {
            vr::VRInput()->TriggerHapticVibrationAction(m_rHand[Left].m_actionHaptic, 0, 1, 4.f,
                                                        1.0f, vr::k_ulInvalidInputValueHandle);
        }
        if (ulHapticDevice == m_rHand[Right].m_source) {
            vr::VRInput()->TriggerHapticVibrationAction(m_rHand[Right].m_actionHaptic, 0, 1, 4.f,
                                                        1.0f, vr::k_ulInvalidInputValueHandle);
        }
    }
...
#error hi
#endif
        if ((true || vr::VRInput()->GetPoseActionDataForNextFrame(m_rHand[eHand].m_actionPose,
            ...
            vr::InputOriginInfo_t originInfo;
            if (vr::VRInput()->GetOriginTrackedDeviceInfo(poseData.activeOrigin, &originInfo,
                                                          sizeof(originInfo)) ==
                vr::VRInputError_None
                && originInfo.trackedDeviceIndex != vr::k_unTrackedDeviceIndexInvalid) {
                std::string sRenderModelName = GetTrackedDeviceString(originInfo.trackedDeviceIndex,
...
//-----------------------------------------------------------------------------
GLuint CMainApplication::CompileGLShader(const char *pchShaderName, const char *pchVertexShader,
                                         const char *pchFragmentShader) {
    ...
    if (vShaderCompiled != GL_TRUE) {
        dprintf("%s - Unable to compile vertex shader %d %d!\n", pchShaderName, nSceneVertexShader,
                glGetError());

        GLint logSize = 0;
        glGetShaderiv(nSceneVertexShader, GL_INFO_LOG_LENGTH, &logSize);
        std::vector<char> errorLog(logSize);
        glGetShaderInfoLog(nSceneVertexShader, logSize, &logSize, errorLog.data());

        dprintf("Shader error output: %s", errorLog.data());
...
bool CMainApplication::CreateAllShaders() {
    m_unSceneProgramID = CompileGLShader(
            ...
            "void main()\n"
            "{\n"
            //"   outputColor = vec4(vec3(LineariseDepth(gl_FragCoord.z) / far), 1.0);"
...
bool CMainApplication::SetupTexturemaps() {
    ...
    std::vector<u_char> pngData = ReadAssetFile("cube_texture.png");
    std::vector<unsigned char> imageRGBA;
    unsigned nImageWidth, nImageHeight;
    // unsigned nError = lodepng::decode(imageRGBA, nImageWidth, nImageHeight, strFullPath.c_str());
    unsigned nError = lodepng::decode(imageRGBA, nImageWidth, nImageHeight, pngData);
...
CMainApplication::CreateFrameBuffer(int nWidth, int nHeight, FramebufferDesc &framebufferDesc) {
    ...
    //                           framebufferDesc.m_nDepthBufferId);
    ...
    glBindTexture(GL_TEXTURE_2D, framebufferDesc.m_nDepthBufferId);
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH24_STENCIL8, nWidth, nHeight);
    ...
    //glTexStorage2DMultisample(GL_TEXTURE_2D, 4, GL_RGBA8, nWidth, nHeight, true);
    // glTexStorage2D(GL_TEXTURE_2D, 4, GL_RGBA8, nWidth, nHeight);
    glTexStorage2D(GL_TEXTURE_2D, 4, GL_RGBA8, nWidth, nHeight);
...
void CMainApplication::SetupCompanionWindow() {
    ...
    glBufferData(GL_ARRAY_BUFFER, vVerts.size() * sizeof(VertexDataWindow), &vVerts[0],
                 GL_STATIC_DRAW);

    glGenBuffers(1, &m_glCompanionWindowIDIndexBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_glCompanionWindowIDIndexBuffer);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_uiCompanionWindowIndexSize * sizeof(GLushort),
...
bool CGLRenderModel::BInit(const vr::RenderModel_t &vrModel,
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vr::RenderModel_Vertex_t) * vrModel.unVertexCount,
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint16_t) * vrModel.unTriangleCount * 3,
