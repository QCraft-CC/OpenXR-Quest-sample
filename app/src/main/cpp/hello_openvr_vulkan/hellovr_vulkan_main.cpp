...
enum DescriptorSetIndex_t {
    ...
    DESCRIPTOR_SET_LEFT_EYE_RENDER_MODEL0 + vr::k_unMaxTrackedDeviceCount,
    DESCRIPTOR_SET_RIGHT_EYE_RENDER_MODEL0,
    DESCRIPTOR_SET_RIGHT_EYE_RENDER_MODEL_MAX =
    DESCRIPTOR_SET_RIGHT_EYE_RENDER_MODEL0 + vr::k_unMaxTrackedDeviceCount,
    NUM_DESCRIPTOR_SETS
};
...
public:
    ...
    bool BInit(VkDevice pDevice, const VkPhysicalDeviceMemoryProperties &memoryProperties,
               VkCommandBuffer pCommandBuffer, vr::TrackedDeviceIndex_t unTrackedDeviceIndex,
               VkDescriptorSet pDescriptorSets[2], const vr::RenderModel_t &vrModel,
               const vr::RenderModel_TextureMap_t &vrDiffuseTexture);

    void Cleanup();

    void Draw(vr::EVREye nEye, VkCommandBuffer pCommandBuffer, VkPipelineLayout pPipelineLayout,
              const Matrix4 &matMVP);
...
public:
    ...
    bool GetVulkanDeviceExtensionsRequired(VkPhysicalDevice pPhysicalDevice,
    ...
    static void GenMipMapRGBA(const uint8_t *pSrc, uint8_t *ppDst, int nSrcWidth, int nSrcHeight,
                              int *pDstWidthOut, int *pDstHeightOut);
    ...
    VulkanRenderModel *FindOrLoadRenderModel(vr::TrackedDeviceIndex_t unTrackedDeviceIndex,
...
void dprintf(const char *fmt, ...) {
    ...
    log(ANDROID_LOG_INFO, "dprintf: " + std::string(buffer));
}
...
VKDebugMessageCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType,
                       uint64_t object,
...
static bool MemoryTypeFromProperties(const VkPhysicalDeviceMemoryProperties &memoryProperties,
    ...
    for (uint32_t i = 0; i < VK_MAX_MEMORY_TYPES; i++) {
        if ((nMemoryTypeBits & 1) == 1) {
            ...
            if ((memoryProperties.memoryTypes[i].propertyFlags & nMemoryProperties) ==
                nMemoryProperties) {
                *pTypeIndexOut = i;
                return true;
            }
...
//-----------------------------------------------------------------------------
static bool
CreateVulkanBuffer(VkDevice pDevice, const VkPhysicalDeviceMemoryProperties &memoryProperties,
                   const void *pBufferData, VkDeviceSize nSize, VkBufferUsageFlags nUsage,
                   VkBuffer *ppBufferOut, VkDeviceMemory *ppDeviceMemoryOut) {
...
CMainApplication::CMainApplication(int argc, char *argv[])
        : //m_pCompanionWindow(NULL), m_nCompanionWindowWidth(640), m_nCompanionWindowHeight(320),
        m_pHMD(NULL), m_pRenderModels(NULL), m_bDebugVulkan(false), m_bVerbose(false),
        m_bPerf(false), m_bVblank(false), m_nMSAASampleCount(4), m_flSuperSampleScale(1.0f),
        m_iTrackedControllerCount(0), m_iTrackedControllerCount_Last(-1), m_iValidPoseCount(0),
        m_iValidPoseCount_Last(-1), m_iSceneVolumeInit(20), m_strPoseClasses(""),
        m_bShowCubes(true), m_pInstance(VK_NULL_HANDLE), m_pDevice(VK_NULL_HANDLE),
        m_pPhysicalDevice(VK_NULL_HANDLE), m_pQueue(VK_NULL_HANDLE), m_pSurface(VK_NULL_HANDLE),
        m_pSwapchain(VK_NULL_HANDLE), m_pDebugReportCallback(VK_NULL_HANDLE),
        m_pCommandPool(VK_NULL_HANDLE), m_pDescriptorPool(VK_NULL_HANDLE),
        m_nSwapQueueImageCount(0), m_nFrameIndex(0), m_nCurrentSwapchainImage(0),
        m_pSceneVertexBuffer(VK_NULL_HANDLE), m_pSceneVertexBufferMemory(VK_NULL_HANDLE),
        m_pSceneVertexBufferView(VK_NULL_HANDLE), m_pSceneImage(VK_NULL_HANDLE),
        m_pSceneImageMemory(VK_NULL_HANDLE), m_pSceneImageView(VK_NULL_HANDLE),
        m_pSceneStagingBuffer(VK_NULL_HANDLE), m_pSceneStagingBufferMemory(VK_NULL_HANDLE),
        m_pSceneSampler(VK_NULL_HANDLE), m_pDescriptorSetLayout(VK_NULL_HANDLE),
        m_pPipelineLayout(VK_NULL_HANDLE), m_pPipelineCache(VK_NULL_HANDLE),
...
std::string GetTrackedDeviceString(vr::IVRSystem *pHmd, vr::TrackedDeviceIndex_t unDevice,
    ...
    uint32_t unRequiredBufferLen = pHmd->GetStringTrackedDeviceProperty(unDevice, prop, NULL, 0,
                                                                        peError);
    if (unRequiredBufferLen == 0)
        return "";

    char *pchBuffer = new char[unRequiredBufferLen];
    unRequiredBufferLen = pHmd->GetStringTrackedDeviceProperty(unDevice, prop, pchBuffer,
...
bool CMainApplication::BInit() {
    ...
    // m_pCompanionWindow = SDL_CreateWindow("hellovr [Vulkan]", nWindowPosX, nWindowPosY,
...
//--------------------------------------------------------------------------------------
bool CMainApplication::GetVulkanInstanceExtensionsRequired(
...
//--------------------------------------------------------------------------------------
bool CMainApplication::GetVulkanDeviceExtensionsRequired(VkPhysicalDevice pPhysicalDevice,
                                                         std::vector<std::string> &outDeviceExtensionList) {
    if (!vr::VRCompositor()) {
        return false;
    }
    ...
    if (nBufferSize > 0) {
        ...
        vr::VRCompositor()->GetVulkanDeviceExtensionsRequired(
                (VkPhysicalDevice_T *) pPhysicalDevice, pExtensionStr, nBufferSize);
...
bool CMainApplication::BInitVulkanInstance() {
        ...
        if (nResult == VK_SUCCESS && nInstanceLayerCount > 0) {
            ...
            for (nLayerIndex = 0; nLayerIndex < nInstanceLayerCount; nLayerIndex++) {
                for (uint32_t nLayer = 0; nLayer < _countof(pInstanceValidationLayers); nLayer++) {
                    if (strstr(pLayerProperties[nLayerIndex].layerName,
                               pInstanceValidationLayers[nLayer]) != NULL) {
    ...
    if (nInstanceExtensionCount > 0) {
        nResult = vkEnumerateInstanceExtensionProperties(NULL, &nInstanceExtensionCount,
        ...
        for (size_t nExt = 0; nExt < requiredInstanceExtensions.size(); nExt++) {
            ...
            for (nExtIndex = 0; nExtIndex < nInstanceExtensionCount; nExtIndex++) {
                if (strcmp(requiredInstanceExtensions[nExt].c_str(),
                           pExtensionProperties[nExtIndex].extensionName) == 0) {
                    bFound = true;
    ...
    if (m_bDebugVulkan) {
        g_pVkCreateDebugReportCallbackEXT = (PFN_vkCreateDebugReportCallbackEXT) vkGetInstanceProcAddr(
                m_pInstance, "vkCreateDebugReportCallbackEXT");
        g_pVkDestroyDebugReportCallbackEXT = (PFN_vkDestroyDebugReportCallbackEXT) vkGetInstanceProcAddr(
                m_pInstance, "vkDestroyDebugReportCallbackEXT");
        ...
        g_pVkCreateDebugReportCallbackEXT(m_pInstance, &debugReportCreateInfo, NULL,
                                          &m_pDebugReportCallback);
    }
...
bool CMainApplication::BInitVulkanDevice() {
    ...
    m_pHMD->GetOutputDevice(&pHMDPhysicalDevice, vr::TextureType_Vulkan,
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(m_pPhysicalDevice, &nQueueCount,
    ...
    nResult = vkEnumerateDeviceExtensionProperties(m_pPhysicalDevice, NULL, &nDeviceExtensionCount,
                                                   NULL);
    if (nResult != VK_SUCCESS) {
    ...
    if (nDeviceExtensionCount > 0) {
        nResult = vkEnumerateDeviceExtensionProperties(m_pPhysicalDevice, NULL,
        ...
        for (size_t nRequiredDeviceExt = 0;
             nRequiredDeviceExt < requiredDeviceExtensions.size(); nRequiredDeviceExt++) {
            bool bExtFound = false;
            for (uint32_t nDeviceExt = 0; nDeviceExt < nDeviceExtensionCount; nDeviceExt++) {
                if (stricmp(requiredDeviceExtensions[nRequiredDeviceExt].c_str(),
...
bool CMainApplication::BInitVulkanSwapchain() {
    ...
    androidSurfaceInfo.window = current_app->window;
    nResult = vkCreateAndroidSurfaceKHR(m_pInstance, &androidSurfaceInfo, nullptr, &m_pSurface);
#else
    ...
    nResult = vkGetPhysicalDeviceSurfaceSupportKHR(m_pPhysicalDevice, m_nQueueFamilyIndex,
                                                   m_pSurface, &bSupportsPresent);
    ...
    if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_pPhysicalDevice, m_pSurface,
                                             &nNumSupportedSwapChainFormats, NULL) != VK_SUCCESS) {
    ...
    if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_pPhysicalDevice, m_pSurface,
                                             &nNumSupportedSwapChainFormats,
                                             pSupportedSurfaceFormats) != VK_SUCCESS) {
        dprintf("Unable to query supported swapchain formats.\n");
        return false;
    }
    if (nNumSupportedSwapChainFormats == 1 &&
        pSupportedSurfaceFormats[0].format == VK_FORMAT_UNDEFINED) {
    ...
    nResult = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_pPhysicalDevice, m_pSurface,
                                                        &surfaceCaps);
    if (nResult != VK_SUCCESS) {
    ...
    nResult = vkGetPhysicalDeviceSurfacePresentModesKHR(m_pPhysicalDevice, m_pSurface,
                                                        &nPresentModeCount, NULL);
    if (nResult != VK_SUCCESS) {
    ...
    nResult = vkGetPhysicalDeviceSurfacePresentModesKHR(m_pPhysicalDevice, m_pSurface,
                                                        &nPresentModeCount, pPresentModes);
    if (nResult != VK_SUCCESS) {
        dprintf("vkGetPhysicalDeviceSurfacePresentModesKHR failed with error %d\n", nResult);
        return false;
    }
    ...
    if (surfaceCaps.currentExtent.width == -1) {
        ...
        dprintf("Could not get surface size\n");
        abort();
    } else {
    ...
    vkGetSwapchainImagesKHR(m_pDevice, m_pSwapchain, &m_nSwapQueueImageCount,
                            &m_swapchainImages[0]);
    if (nResult != VK_SUCCESS) {
        dprintf("vkGetSwapchainImagesKHR failed with error %d\n", nResult);
        return false;
...
void CMainApplication::Shutdown() {
    ...
    for (std::vector<VulkanRenderModel *>::iterator i = m_vecRenderModels.begin();
    ...
    if (m_pDevice != VK_NULL_HANDLE) {
        for (std::deque<VulkanCommandBuffer_t>::iterator i = m_commandBuffers.begin();
             i != m_commandBuffers.end(); i++) {
        ...
        for (int32_t nFramebuffer = 0; nFramebuffer < 2; nFramebuffer++) {
            if (pFramebufferDescs[nFramebuffer]->m_pImageView != VK_NULL_HANDLE) {
                vkDestroyImageView(m_pDevice, pFramebufferDescs[nFramebuffer]->m_pImageView,
                                   nullptr);
                ...
                vkDestroyImageView(m_pDevice,
                                   pFramebufferDescs[nFramebuffer]->m_pDepthStencilImageView,
                                   nullptr);
                vkDestroyImage(m_pDevice, pFramebufferDescs[nFramebuffer]->m_pDepthStencilImage,
                               nullptr);
                vkFreeMemory(m_pDevice,
                             pFramebufferDescs[nFramebuffer]->m_pDepthStencilDeviceMemory, nullptr);
                vkDestroyRenderPass(m_pDevice, pFramebufferDescs[nFramebuffer]->m_pRenderPass,
                                    nullptr);
                vkDestroyFramebuffer(m_pDevice, pFramebufferDescs[nFramebuffer]->m_pFramebuffer,
                                     nullptr);
            }
        }
        ...
        for (size_t nSwapchainIndex = 0;
             nSwapchainIndex < m_pSwapchainFramebuffers.size(); nSwapchainIndex++) {
...
bool CMainApplication::CreateAllShaders() {
    ...
    for (int32_t nShader = 0; nShader < PSO_COUNT; nShader++) {
        for (int32_t nStage = 0; nStage <= 1; nStage++) {
            ...
            sprintf(shaderFileName, "../shaders/%s_%s.spv", pShaderNames[nShader],
                    pStageNames[nStage]);
            ...
            */
            ...
            char shaderFileName[1024];
            sprintf(shaderFileName, "hello_openvr_vulkan_shaders/%s_%s.spv", pShaderNames[nShader],
                    pStageNames[nStage]);
            std::vector<uint8_t> contents = ReadAssetFile(shaderFileName);
            const char *pBuffer = (const char *) contents.data();
            size_t nSize = contents.size();
            ...
            nResult = vkCreateShaderModule(m_pDevice, &shaderModuleCreateInfo, nullptr,
                                           &m_pShaderModules[nShader * 2 + nStage]);
            if (nResult != VK_SUCCESS) {
                dprintf("Error creating shader module for %s, error %d\n",
                        shaderFileName, // shaderPath.c_str(),
                        nResult);
                return false;
            }
    ...
    nResult = vkCreateDescriptorSetLayout(m_pDevice, &descriptorSetLayoutCreateInfo, nullptr,
    ...
    nResult = vkCreatePipelineLayout(m_pDevice, &pipelineLayoutCreateInfo, nullptr,
                                     &m_pPipelineLayout);
    if (nResult != VK_SUCCESS) {
    ...
    VkVertexInputAttributeDescription attributeDescriptions[PSO_COUNT * 3]
            {
                    ...
                    {2, 0, VK_FORMAT_R32G32_SFLOAT,    offsetof(vr::RenderModel_Vertex_t,
                                                                rfTextureCoord)},
                    // PSO_COMPANION
    ...
    for (uint32_t nPSO = 0; nPSO < PSO_COUNT; nPSO++) {
        ...
        iaState.topology = (nPSO == PSO_AXES) ? VK_PRIMITIVE_TOPOLOGY_LINE_LIST
        ...
        msState.rasterizationSamples = (nPSO == PSO_COMPANION) ? VK_SAMPLE_COUNT_1_BIT
                                                               : (VkSampleCountFlagBits) m_nMSAASampleCount;
        ...
        // Create the pipeline
        nResult = vkCreateGraphicsPipelines(m_pDevice, m_pPipelineCache, 1, &pipelineCreateInfo,
                                            NULL, &m_pPipelines[nPSO]);
        if (nResult != VK_SUCCESS) {
...
void CMainApplication::CreateAllDescriptorSets() {
    ...
    for (uint32_t nEye = 0; nEye < 2; nEye++) {
        ...
        vkUpdateDescriptorSets(m_pDevice, _countof(writeDescriptorSets), writeDescriptorSets, 0,
                               nullptr);
    }
...
bool CMainApplication::SetupTexturemaps() {
    ...
    MemoryTypeFromProperties(m_physicalDeviceMemoryProperties, memoryRequirements.memoryTypeBits,
    ...
    if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, pBuffer, nBufferSize,
                            VK_BUFFER_USAGE_TRANSFER_SRC_BIT, &m_pSceneStagingBuffer,
    ...
    vkCmdPipelineBarrier(m_currentCommandBuffer.m_pCommandBuffer,
                         VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0,
                         NULL, 0, NULL, 1, &imageMemoryBarrier);

    // Issue the copy to fill the image data
    vkCmdCopyBufferToImage(m_currentCommandBuffer.m_pCommandBuffer, m_pSceneStagingBuffer,
                           m_pSceneImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           (uint32_t) bufferImageCopies.size(), &bufferImageCopies[0]);
    ...
    vkCmdPipelineBarrier(m_currentCommandBuffer.m_pCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
...
//-----------------------------------------------------------------------------
void
CMainApplication::GenMipMapRGBA(const uint8_t *pSrc, uint8_t *pDst, int nSrcWidth, int nSrcHeight,
                                int *pDstWidthOut, int *pDstHeightOut) {
...
void CMainApplication::SetupScene() {
    ...
    for (int z = 0; z < m_iSceneVolumeDepth; z++) {
        for (int y = 0; y < m_iSceneVolumeHeight; y++) {
            ...
            mat = mat * Matrix4().translate(-((float) m_iSceneVolumeWidth) * m_fScaleSpacing,
                                            m_fScaleSpacing, 0);
        }
        mat = mat * Matrix4().translate(0, -((float) m_iSceneVolumeHeight) * m_fScaleSpacing,
                                        m_fScaleSpacing);
    }
    m_uiVertcount = vertdataarray.size() / 5;

    // Create the vertex buffer and fill with data
    if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, &vertdataarray[0],
                            vertdataarray.size() * sizeof(float),
                            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &m_pSceneVertexBuffer,
                            &m_pSceneVertexBufferMemory)) {
        return;
    }
    ...
    for (uint32_t nEye = 0; nEye < 2; nEye++) {
        ...
        VkMemoryAllocateInfo allocInfo = {VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
        MemoryTypeFromProperties(m_physicalDeviceMemoryProperties,
                                 memoryRequirements.memoryTypeBits,
                                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
                                 VK_MEMORY_PROPERTY_HOST_CACHED_BIT, &allocInfo.memoryTypeIndex);
        allocInfo.allocationSize = memoryRequirements.size;

        vkAllocateMemory(m_pDevice, &allocInfo, nullptr, &m_pSceneConstantBufferMemory[nEye]);
        vkBindBufferMemory(m_pDevice, m_pSceneConstantBuffer[nEye],
                           m_pSceneConstantBufferMemory[nEye], 0);

        // Map and keep mapped persistently
        vkMapMemory(m_pDevice, m_pSceneConstantBufferMemory[nEye], 0, VK_WHOLE_SIZE, 0,
                    &m_pSceneConstantBufferData[nEye]);
    }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainApplication::AddCubeVertex(float fl0, float fl1, float fl2, float fl3, float fl4,
                                     std::vector<float> &vertdata) {
...
void CMainApplication::UpdateControllerAxes() {
    ...
    if (m_pControllerAxesVertexBuffer == VK_NULL_HANDLE && vertdataarray.size() > 0) {
        ...
        if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, nullptr, nSize,
                                VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &m_pControllerAxesVertexBuffer,
                                &m_pControllerAxesVertexBufferMemory)) {
            return;
        }
    }
    ...
    if (m_pControllerAxesVertexBuffer != VK_NULL_HANDLE && vertdataarray.size() > 0) {
        void *pData;
        VkResult nResult = vkMapMemory(m_pDevice, m_pControllerAxesVertexBufferMemory, 0,
                                       VK_WHOLE_SIZE, 0, &pData);
        if (nResult != VK_SUCCESS) {
...
//-----------------------------------------------------------------------------
bool
CMainApplication::CreateFrameBuffer(int nWidth, int nHeight, FramebufferDesc &framebufferDesc) {
    //---------------------------//
    //    Create color target    //
    //---------------------------//
    VkImageCreateInfo imageCreateInfo = {VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};
    ...
    VkMemoryAllocateInfo memoryAllocateInfo = {VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
    memoryAllocateInfo.allocationSize = memoryRequirements.size;
    if (!MemoryTypeFromProperties(m_physicalDeviceMemoryProperties,
                                  memoryRequirements.memoryTypeBits,
                                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                  &memoryAllocateInfo.memoryTypeIndex)) {
    ...
    nResult = vkAllocateMemory(m_pDevice, &memoryAllocateInfo, nullptr,
                               &framebufferDesc.m_pDeviceMemory);
    if (nResult != VK_SUCCESS) {
        dprintf("Failed to find memory for image.\n");
        return false;
    }

    nResult = vkBindImageMemory(m_pDevice, framebufferDesc.m_pImage,
                                framebufferDesc.m_pDeviceMemory, 0);
    if (nResult != VK_SUCCESS) {
    ...
    imageViewCreateInfo.subresourceRange.baseArrayLayer = 0;
    imageViewCreateInfo.subresourceRange.layerCount = 1;
    nResult = vkCreateImageView(m_pDevice, &imageViewCreateInfo, nullptr,
                                &framebufferDesc.m_pImageView);
    if (nResult != VK_SUCCESS) {
    ...
    nResult = vkCreateImage(m_pDevice, &imageCreateInfo, nullptr,
                            &framebufferDesc.m_pDepthStencilImage);
    if (nResult != VK_SUCCESS) {
        dprintf("vkCreateImage failed for eye depth buffer with error %d\n", nResult);
        return false;
    }
    vkGetImageMemoryRequirements(m_pDevice, framebufferDesc.m_pDepthStencilImage,
                                 &memoryRequirements);
    ...
    nResult = vkAllocateMemory(m_pDevice, &memoryAllocateInfo, nullptr,
                               &framebufferDesc.m_pDepthStencilDeviceMemory);
    if (nResult != VK_SUCCESS) {
        dprintf("Failed to find memory for image.\n");
        return false;
    }

    nResult = vkBindImageMemory(m_pDevice, framebufferDesc.m_pDepthStencilImage,
                                framebufferDesc.m_pDepthStencilDeviceMemory, 0);
    if (nResult != VK_SUCCESS) {
        dprintf("Failed to bind memory for image.\n");
        return false;
    }
    ...
    nResult = vkCreateImageView(m_pDevice, &imageViewCreateInfo, nullptr,
                                &framebufferDesc.m_pDepthStencilImageView);
    if (nResult != VK_SUCCESS) {
        dprintf("vkCreateImageView failed with error %d\n", nResult);
        return false;
    }
    ...
    nResult = vkCreateRenderPass(m_pDevice, &renderPassCreateInfo, NULL,
                                 &framebufferDesc.m_pRenderPass);
    if (nResult != VK_SUCCESS) {
        dprintf("vkCreateRenderPass failed with error %d.\n", nResult);
        return false;
    }
    ...
    VkImageView attachments[2] = {framebufferDesc.m_pImageView,
                                  framebufferDesc.m_pDepthStencilImageView};
    VkFramebufferCreateInfo framebufferCreateInfo = {VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO};
    ...
    framebufferCreateInfo.layers = 1;
    nResult = vkCreateFramebuffer(m_pDevice, &framebufferCreateInfo, NULL,
                                  &framebufferDesc.m_pFramebuffer);
    if (nResult != VK_SUCCESS) {
        dprintf("vkCreateFramebuffer failed with error %d.\n", nResult);
        return false;
    }
...
CMainApplication::VulkanCommandBuffer_t CMainApplication::GetCommandBuffer() {
    ...
    if (m_commandBuffers.size() > 0) {
        ...
        if (vkGetFenceStatus(m_pDevice, m_commandBuffers.back().m_pFence) == VK_SUCCESS) {
            ...
            vkResetCommandBuffer(commandBuffer.m_pCommandBuffer,
    ...
    vkAllocateCommandBuffers(m_pDevice, &commandBufferAllocateInfo,
...
void CMainApplication::SetupCompanionWindow() {
    ...
    // Create the vertex buffer and fill with data
    if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, &vVerts[0],
                            sizeof(VertexDataWindow) * vVerts.size(),
                            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &m_pCompanionWindowVertexBuffer,
                            &m_pCompanionWindowVertexBufferMemory)) {
        return;
    }
    ...
    if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, &vIndices[0],
                            sizeof(vIndices),
                            VK_BUFFER_USAGE_INDEX_BUFFER_BIT, &m_pCompanionWindowIndexBuffer,
                            &m_pCompanionWindowIndexBufferMemory)) {
        return;
    }

    // Transition all of the swapchain images to PRESENT_SRC so they are ready for presentation
    for (size_t nSwapchainImage = 0;
         nSwapchainImage < m_swapchainImages.size(); nSwapchainImage++) {
        VkImageMemoryBarrier imageMemoryBarrier = {VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};
        imageMemoryBarrier.srcAccessMask = 0;
...
void CMainApplication::RenderStereoTargets() {
    ...
    vkCmdBeginRenderPass(m_currentCommandBuffer.m_pCommandBuffer, &renderPassBeginInfo,
...
void CMainApplication::RenderScene(vr::Hmd_Eye nEye) {
    if (m_bShowCubes) {
        vkCmdBindPipeline(m_currentCommandBuffer.m_pCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          m_pPipelines[PSO_SCENE]);

        // Update the persistently mapped pointer to the CB data with the latest matrix
        memcpy(m_pSceneConstantBufferData[nEye], GetCurrentViewProjectionMatrix(nEye).get(),
               sizeof(Matrix4));

        vkCmdBindDescriptorSets(m_currentCommandBuffer.m_pCommandBuffer,
                                VK_PIPELINE_BIND_POINT_GRAPHICS, m_pPipelineLayout, 0, 1,
                                &m_pDescriptorSets[DESCRIPTOR_SET_LEFT_EYE_SCENE + nEye], 0,
                                nullptr);

        // Draw
        VkDeviceSize nOffsets[1] = {0};
        vkCmdBindVertexBuffers(m_currentCommandBuffer.m_pCommandBuffer, 0, 1, &m_pSceneVertexBuffer,
                               &nOffsets[0]);
    ...
    if (bIsInputAvailable && m_pControllerAxesVertexBuffer != VK_NULL_HANDLE) {
        ...
        vkCmdBindPipeline(m_currentCommandBuffer.m_pCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          m_pPipelines[PSO_AXES]);

        VkDeviceSize nOffsets[1] = {0};
        vkCmdBindVertexBuffers(m_currentCommandBuffer.m_pCommandBuffer, 0, 1,
                               &m_pControllerAxesVertexBuffer, &nOffsets[0]);
        vkCmdDraw(m_currentCommandBuffer.m_pCommandBuffer, m_uiControllerVertcount, 1, 0, 0);
    }

    // ----- Render Model rendering -----
    vkCmdBindPipeline(m_currentCommandBuffer.m_pCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      m_pPipelines[PSO_RENDERMODEL]);
    for (uint32_t unTrackedDevice = 0;
        ...
        m_rTrackedDeviceToRenderModel[unTrackedDevice]->Draw(nEye,
                                                             m_currentCommandBuffer.m_pCommandBuffer,
                                                             m_pPipelineLayout, matMVP);
    }
}
...
void CMainApplication::RenderCompanionWindow() {
    ...
    VkResult nResult = vkAcquireNextImageKHR(m_pDevice, m_pSwapchain, UINT64_MAX,
                                             m_pSwapchainSemaphores[m_nFrameIndex], VK_NULL_HANDLE,
                                             &m_nCurrentSwapchainImage);
    if (nResult != VK_SUCCESS) {
    ...
    VkViewport viewport = {0.0f, 0.0f, (float) m_nCompanionWindowWidth,
                           (float) m_nCompanionWindowHeight, 0.0f, 1.0f};
    vkCmdSetViewport(m_currentCommandBuffer.m_pCommandBuffer, 0, 1, &viewport);
    ...
    vkCmdBindPipeline(m_currentCommandBuffer.m_pCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      m_pPipelines[PSO_COMPANION]);
    vkCmdBindDescriptorSets(m_currentCommandBuffer.m_pCommandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, m_pPipelineLayout, 0, 1,
                            &m_pDescriptorSets[DESCRIPTOR_SET_COMPANION_LEFT_TEXTURE], 0, nullptr);

    // Draw left eye texture to companion window
    VkDeviceSize nOffsets[1] = {0};
    vkCmdBindVertexBuffers(m_currentCommandBuffer.m_pCommandBuffer, 0, 1,
                           &m_pCompanionWindowVertexBuffer, &nOffsets[0]);
    vkCmdBindIndexBuffer(m_currentCommandBuffer.m_pCommandBuffer, m_pCompanionWindowIndexBuffer, 0,
                         VK_INDEX_TYPE_UINT16);
    vkCmdDrawIndexed(m_currentCommandBuffer.m_pCommandBuffer, m_uiCompanionWindowIndexSize / 2, 1,
                     0, 0, 0);

    // Draw right eye texture to companion window
    vkCmdBindDescriptorSets(m_currentCommandBuffer.m_pCommandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, m_pPipelineLayout, 0, 1,
                            &m_pDescriptorSets[DESCRIPTOR_SET_COMPANION_RIGHT_TEXTURE], 0, nullptr);
    vkCmdDrawIndexed(m_currentCommandBuffer.m_pCommandBuffer, m_uiCompanionWindowIndexSize / 2, 1,
                     (m_uiCompanionWindowIndexSize / 2), 0, 0);
...
//-----------------------------------------------------------------------------
VulkanRenderModel *
CMainApplication::FindOrLoadRenderModel(vr::TrackedDeviceIndex_t unTrackedDeviceIndex,
                                        const char *pchRenderModelName) {
    ...
    if (!pRenderModel) {
        ...
        VkDescriptorSet pDescriptorSets[2] =
                {
                        m_pDescriptorSets[DESCRIPTOR_SET_LEFT_EYE_RENDER_MODEL0 +
                                          unTrackedDeviceIndex],
                        m_pDescriptorSets[DESCRIPTOR_SET_RIGHT_EYE_RENDER_MODEL0 +
                                          unTrackedDeviceIndex],
                };
        ...
        if (!pRenderModel->BInit(m_pDevice, m_physicalDeviceMemoryProperties,
                                 m_currentCommandBuffer.m_pCommandBuffer, unTrackedDeviceIndex,
                                 pDescriptorSets, *pModel, *pTexture)) {
...
//-----------------------------------------------------------------------------
void
CMainApplication::SetupRenderModelForTrackedDevice(vr::TrackedDeviceIndex_t unTrackedDeviceIndex) {
    if (unTrackedDeviceIndex >= vr::k_unMaxTrackedDeviceCount)
        return;

    // try to find a model we've already set up
    std::string sRenderModelName = GetTrackedDeviceString(m_pHMD, unTrackedDeviceIndex,
                                                          vr::Prop_RenderModelName_String);
    VulkanRenderModel *pRenderModel = FindOrLoadRenderModel(unTrackedDeviceIndex,
                                                            sRenderModelName.c_str());
    if (!pRenderModel) {
        std::string sTrackingSystemName = GetTrackedDeviceString(m_pHMD, unTrackedDeviceIndex,
                                                                 vr::Prop_TrackingSystemName_String);
        dprintf("Unable to load render model for tracked device %d (%s.%s)", unTrackedDeviceIndex,
...
VulkanRenderModel::VulkanRenderModel(const std::string &sRenderModelName)
        : m_sModelName(sRenderModelName), m_pDevice(VK_NULL_HANDLE),
          m_pVertexBuffer(VK_NULL_HANDLE), m_pVertexBufferMemory(VK_NULL_HANDLE),
          m_pIndexBuffer(VK_NULL_HANDLE), m_pIndexBufferMemory(VK_NULL_HANDLE),
          m_pImage(VK_NULL_HANDLE), m_pImageMemory(VK_NULL_HANDLE), m_pImageView(VK_NULL_HANDLE),
          m_pImageStagingBuffer(VK_NULL_HANDLE), m_pImageStagingBufferMemory(VK_NULL_HANDLE),
...
//-----------------------------------------------------------------------------
bool
VulkanRenderModel::BInit(VkDevice pDevice, const VkPhysicalDeviceMemoryProperties &memoryProperties,
    ...
    {
        if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, vrModel.rVertexData,
                                sizeof(vr::RenderModel_Vertex_t) * vrModel.unVertexCount,
                                VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &m_pVertexBuffer,
    ...
    {
        if (!CreateVulkanBuffer(m_pDevice, m_physicalDeviceMemoryProperties, vrModel.rIndexData,
                                sizeof(uint16_t) * vrModel.unTriangleCount * 3,
                                VK_BUFFER_USAGE_INDEX_BUFFER_BIT, &m_pIndexBuffer,
    ...
    // create and populate the texture
    {
        ...
        uint8_t *pPrevBuffer = pBuffer;
        uint8_t *pCurBuffer = pBuffer;
        memcpy(pCurBuffer, vrDiffuseTexture.rubTextureMapData,
               sizeof(uint8_t) * nImageWidth * nImageHeight * 4);
        pCurBuffer += sizeof(uint8_t) * nImageWidth * nImageHeight * 4;
        ...
        while (nMipWidth > 1 && nMipHeight > 1) {
            CMainApplication::GenMipMapRGBA(pPrevBuffer, pCurBuffer, nMipWidth, nMipHeight,
        ...
        void *pData;
        VkResult nResult = vkMapMemory(m_pDevice, m_pImageStagingBufferMemory, 0, VK_WHOLE_SIZE, 0,
                                       &pData);
        if (nResult != VK_SUCCESS) {
            dprintf("vkMapMemory returned error %d\n", nResult);
            return false;
        ...
        imageMemoryBarrier.subresourceRange.baseArrayLayer = 0;
        imageMemoryBarrier.subresourceRange.layerCount = 1;
        vkCmdPipelineBarrier(pCommandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                             VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1,
                             &imageMemoryBarrier);

        // Issue the copy to fill the image data
        vkCmdCopyBufferToImage(pCommandBuffer, m_pImageStagingBuffer, m_pImage,
        ...
        imageMemoryBarrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
        imageMemoryBarrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        vkCmdPipelineBarrier(pCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                             VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1,
                             &imageMemoryBarrier);
    ...
    for (uint32_t nEye = 0; nEye < 2; nEye++) {
        ...
        // Map and keep mapped persistently
        vkMapMemory(m_pDevice, m_pConstantBufferMemory[nEye], 0, VK_WHOLE_SIZE, 0,
                    &m_pConstantBufferData[nEye]);
...
void VulkanRenderModel::Cleanup() {
    ...
    for (uint32_t nEye = 0; nEye < 2; nEye++) {
        ...
        if (m_pConstantBufferMemory[nEye] != VK_NULL_HANDLE) {
...
//-----------------------------------------------------------------------------
void VulkanRenderModel::Draw(vr::EVREye nEye, VkCommandBuffer pCommandBuffer,
                             VkPipelineLayout pPipelineLayout, const Matrix4 &matMVP) {
    // Update the CB with the transform
    memcpy(m_pConstantBufferData[nEye], &matMVP, sizeof(matMVP));

    // Bind the descriptor set
    vkCmdBindDescriptorSets(pCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pPipelineLayout, 0, 1,
                            &m_pDescriptorSets[nEye], 0, nullptr);
